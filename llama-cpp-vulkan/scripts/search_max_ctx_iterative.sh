...
RAMP_FACTOR_PCT="${RAMP_FACTOR_PCT:-125}"
MIN_RAMP_STEP="${MIN_RAMP_STEP:-8192}"
MAX_RAMP_STEP="${MAX_RAMP_STEP:-65536}"
...
echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] model=${MODEL_ALIAS} strategy=iterative_then_binary min=${min_ctx_rounded} cap=${cap_ctx_rounded} precision=${PRECISION} max_tokens=${MAX_TOKENS}"
...
if ! run_probe "$min_ctx_rounded"; then
  {
    ...
    echo "strategy=iterative_then_binary"
...
while [ "$best_ok" -lt "$cap_ctx_rounded" ]; do
  jump=$(( best_ok * (RAMP_FACTOR_PCT - 100) / 100 ))
  if [ "$jump" -lt "$MIN_RAMP_STEP" ]; then
    jump="$MIN_RAMP_STEP"
  fi
  if [ "$jump" -gt "$MAX_RAMP_STEP" ]; then
    jump="$MAX_RAMP_STEP"
  fi

  candidate=$((best_ok + jump))
  candidate="$(to_k_step "$candidate" "$PRECISION")"
  if [ "$candidate" -gt "$cap_ctx_rounded" ]; then
    candidate="$cap_ctx_rounded"
  fi
  if [ "$candidate" -le "$best_ok" ]; then
    break
  fi

  if run_probe "$candidate"; then
    best_ok="$candidate"
    continue
  fi
...
while [ "$first_fail" -le "$cap_ctx_rounded" ] && [ $((first_fail - best_ok)) -gt "$PRECISION" ]; do
...

{
  ...
  echo "precision=${PRECISION}"
  echo "strategy=iterative_then_binary"
  echo "ramp_factor_pct=${RAMP_FACTOR_PCT}"
  echo "min_ramp_step=${MIN_RAMP_STEP}"
  echo "max_ramp_step=${MAX_RAMP_STEP}"
