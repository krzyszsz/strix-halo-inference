...
CONTAINER="llama-qwen3-coder-next-test"
...
curl -s "http://127.0.0.1:${PORT}/v1/chat/completions" \
  ...
  -d '{
    "model": "local-gguf",
    "messages": [{"role": "user", "content": "Write a short Python function that reverses a linked list."}],
    "temperature": 0.7,
    "top_p": 0.8,
    "top_k": 20,
    "max_tokens": 256
  }' > "$OUT_DIR/qwen3_coder_next_q5.json"

docker rm -f "$CONTAINER" >/dev/null 2>&1 || true

echo "Saved response to $OUT_DIR/qwen3_coder_next_q5.json"
