...
MODEL="$MODEL_ROOT/qwen3-32b-gguf/Qwen3-32B-Q6_K.gguf"
PORT="${PORT:-8006}"
CTX_SIZE="${CTX_SIZE:-4096}"
GPU_LAYERS="${GPU_LAYERS:-999}"
THREADS="${THREADS:-8}"
OUT_DIR="$REPO_ROOT/llama-cpp-vulkan/out"
CONTAINER="llama-qwen3-32b-test"
EXTRA_ARGS="${EXTRA_ARGS:---reasoning-budget 0}"
...
curl -s "http://127.0.0.1:${PORT}/v1/chat/completions" \
  ...
  -d '{
    "model": "local-gguf",
    "messages": [{"role": "user", "content": "Summarize the key differences between REST and gRPC."}],
    "temperature": 0.7,
    "top_p": 0.8,
    "top_k": 20,
    "max_tokens": 256
  }' > "$OUT_DIR/qwen3_32b_q6.json"

docker rm -f "$CONTAINER" >/dev/null 2>&1 || true

echo "Saved response to $OUT_DIR/qwen3_32b_q6.json"
