...
CONTAINER="llama-qwen25-coder-test"
...
curl -s "http://127.0.0.1:${PORT}/v1/chat/completions" \
  ...
  -d '{
    "model": "local-gguf",
    "messages": [{"role": "user", "content": "Write a Python function that returns the Fibonacci sequence up to n, with a short explanation."}],
    "temperature": 0.7,
    "top_p": 0.8,
    "top_k": 20,
    "max_tokens": 256
  }' > "$OUT_DIR/qwen25_coder_32b.json"

docker rm -f "$CONTAINER" >/dev/null 2>&1 || true

echo "Saved response to $OUT_DIR/qwen25_coder_32b.json"
