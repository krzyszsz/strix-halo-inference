...
MODEL="${MODEL:-$MODEL_ROOT/gpt-oss-120b-gguf/gpt-oss-120b-mxfp4-00001-of-00003.gguf}"
PORT="${PORT:-8005}"
CTX_SIZE="${CTX_SIZE:-2048}"
GPU_LAYERS="${GPU_LAYERS:-999}"
THREADS="${THREADS:-8}"
OUT_DIR="$REPO_ROOT/llama-cpp-vulkan/out"
OUT_PATH="${OUT_PATH:-$OUT_DIR/gpt_oss_120b_mxfp4.json}"
CONTAINER="llama-gpt-oss-120b-test"
...
curl -s "http://127.0.0.1:${PORT}/v1/chat/completions" \
  ...
  -d "{
    ...
    \"messages\": [
      ...
      {\"role\": \"user\", \"content\": \"In 5 bullet points, explain how to benchmark a local LLM server reproducibly.\"}
    ],
    \"temperature\": 0.7,
    \"top_p\": 1.0,
...
echo "Saved response to $OUT_PATH"
