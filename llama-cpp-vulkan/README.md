...
- `scripts/download_gemma4_26b_a4b_gguf.sh`: download Gemma 4 26B-A4B GGUF quants + mmproj and verify SHA256.
  - Quant selection flags: `DOWNLOAD_Q8=1|0`, `DOWNLOAD_Q4=1|0`.
  - Default transfer backend is checksum-enforced `aria2` (when installed); fallback transport: `DIRECT_HTTP=1` (direct `wget -c` path).
- `scripts/run_gemma4_vlm_task_suite.sh`: Gemma4 task suite (chat/QA + coding + image description + object detection JSON output).
- `scripts/check_gemma4_ctx_matrix.sh`: context sweep for Gemma4 (`32k`, `64k`, `128k`, `168k`, `256k`) with configurable primary/fallback labels (`PRIMARY_LABEL`, `FALLBACK_LABEL`) and GPU/CPU retries.
- `scripts/run_gemma4_context_compaction_demo.sh`: local context-compaction demo (baseline full-history vs compacted-memory flow) for coding and non-coding long chats.
