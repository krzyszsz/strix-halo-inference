...
MODEL_ID="$MODEL_ROOT/qwen-image-tiny-random"
DTYPE="${DTYPE:-float32}"
PORT="${PORT:-8000}"
CONTAINER="qwen-image-tiny-test"
OUT_DIR="$REPO_ROOT/qwen-image/out"
OUT_PATH="$OUT_DIR/qwen_image_test.png"
MEM_LIMIT="${MEM_LIMIT:-75g}"
MEMORY_SWAP="${MEMORY_SWAP:-75g}"
MEM_RESERVATION="${MEM_RESERVATION:-67g}"
OOM_SCORE_ADJ="${OOM_SCORE_ADJ:-500}"
RESP_JSON="$(mktemp "${TMPDIR:-/tmp}/qwen_image_tiny.XXXXXX.json")"
trap 'rm -f "$RESP_JSON"' EXIT
...
curl -s "http://127.0.0.1:${PORT}/v1/images/generations" \
