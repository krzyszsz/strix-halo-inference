...
ENABLE_ATTENTION_SLICING = os.environ.get("ATTENTION_SLICING", "0") == "1"
...
def _get_pipeline(model_id: str, dtype: str) -> DiffusionPipeline:
    ...
    if os.environ.get("VAE_SLICING", "1") == "1":
        if hasattr(getattr(pipe, "vae", None), "enable_slicing"):
            pipe.vae.enable_slicing()
        elif hasattr(pipe, "enable_vae_slicing"):
            pipe.enable_vae_slicing()
    if os.environ.get("VAE_TILING", "0") == "1":
...
def _filter_supported_kwargs(pipe: DiffusionPipeline, kwargs: Dict[str, Any]) -> Dict[str, Any]:
    try:
...
def generate(request: GenerationRequest) -> GenerationResponse:
    ...
    kwargs = _filter_supported_kwargs(pipe, _extract_kwargs(request.parameters))
